...
export default function TextForm(props) {
    ...
    const handleAutoCap = ()=>{
        if(text){
        let myArr = text.split(/\s+/);
        let newText = myArr.map((word) => {
  ...
  return (
    ...
    <button className="btn btn-secondary my-2 mx-2" disabled={text.length===0} onClick={handleAutoCap}>Auto-Capitalize</button>
